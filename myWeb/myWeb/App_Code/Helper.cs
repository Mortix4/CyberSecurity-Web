...
namespace myWeb
{
    public class Helper
    {
        ...
        public const string DBName = "myDB.mdf";   //Name of the MSSQL Database.
        ...
        // Delets all the users in the array "userIdToDelete".
        {
            ...
            {
                {
                    DataRow[] dr = ds.Tables[tblName].Select($"Id = {userIdToDelete[i]}");
                    dr[0].Delete();
                }
            }
        ...
        { //פעולה שתפקידה להחזיר את שמו הפרטי של המשתמש
            ...
            DataTable table = Helper.ExecuteDataTable(fileName, sql);
            conn.Close();
            return (bool)table.Rows[0]["isAdmin"];//מטבע הדברים השאילתא החזירה שורה אחת שעונה על התנאים
        }
        
        public static bool GetGender(string fileName, string sql)
        { //פעולה שתפקידה להחזיר את שמו הפרטי של המשתמש
            SqlConnection conn = ConnectToDb(fileName);
            conn.Open();
            SqlCommand com = new SqlCommand(sql, conn);
            ...
            DataTable table = Helper.ExecuteDataTable(fileName, sql);
            conn.Close();
            return (bool)table.Rows[0]["gender"];//מטבע הדברים השאילתא החזירה שורה אחת שעונה על התנאים
